            ...
            for name in ['op', 'lfr', 'rw', 'dir', 'pr', 'ad', 'mfc1', 'mfc2', 'roc1', 'roc2', 'roc3', 'ceo1', 'ceo2', 'ceo3', 'eo','fairway', 'fairsmote']:
                if data[algo][z][k][name] != '/':
                    ...
                    if mann(default_list, real_list) >= 0.05 or rise_ratio >= 0:
                        diff_degree[k]['noorincrease'] += 1
                    elif cohenn < 0.5:
                        diff_degree[k]['small'] += 1
                    elif cohenn >= 0.5 and cohenn < 0.8:
                        diff_degree[k]['medium'] += 1
                    elif cohenn >= 0.8:
                        diff_degree[k]['large'] += 1

fout = open("fig5_result", 'w')
for name in metric_list:
    fout.write(name)
    summ = diff_degree[name]['noorincrease'] + diff_degree[name]['small'] + diff_degree[name]['medium'] + diff_degree[name]['large']
    for j in ['noorincrease', 'small', 'medium', 'large']:
